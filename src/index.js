...
const firstGenerateItem = (mass, inputName) => {
    let myNode = document.getElementById('main-section')
    while(myNode.firstChild) {
        myNode.removeChild(myNode.firstChild)
    }
    sortObjectA(mass)
    const nonFav = mass.filter(item => item.favorite === 'none')
    const yesFav = mass.filter(item => item.favorite === 'favorite')
    
    
    console.log(mass)
    secondGenerateItem(yesFav, inputName)
    secondGenerateItem(nonFav, inputName)
}

const secondGenerateItem = (mass, inputName) => {
    
    mass.forEach((item)=>{
        const onClickDel = () => {
            let tmpId = contacts.map(e => e.id).indexOf(item.id)
            if(tmpId !== 0)
            {
                contacts.splice(tmpId,tmpId)
            }
...
const searchOnPress = () => {
    ...
    firstGenerateItem(contacts, inputName)
}
